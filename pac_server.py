...
GFW_PAC = """// Generated by gfwlist2pac
...
ALL_PAC = """function FindProxyForURL(url, host)
{
...
class PacHandler(tornado.web.RequestHandler):
    ...
    def get(self):
        self.write(GFW_PAC.replace("127.0.0.1:1080", self.request.host.split(':')[0]+":1080"))

class PacAllHandler(tornado.web.RequestHandler):

    def get(self):
        self.write(ALL_PAC.replace("127.0.0.1:1080", self.request.host.split(':')[0]+":1080"))

def startPacServer():
    application = tornado.web.Application([
        (r"/part.pac", PacHandler),
        (r"/all.pac", PacAllHandler),
    ])
    application.listen(8101)
    tornado.ioloop.IOLoop.instance().start()


import threading
class PacServerThread(threading.Thread):
    instance = None
    def __init__(self):
        super(PacServerThread, self).__init__()
        PacServerThread.instance = self
    ...
    def stop(self):
        tornado.ioloop.IOLoop.instance().add_callback(self.stop_ex)

def asyncStart():
    PacServerThread().start()

def asyncStop():
    if PacServerThread.instance is not None:
        PacServerThread.instance.stop()
